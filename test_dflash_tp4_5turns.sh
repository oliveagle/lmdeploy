#!/bin/bash
# DFlash TP=4 + TurboQuant 测试 - 5轮对话
#
# 使用 TP=4 + TurboQuant KV cache 量化来减少显存占用:
# - TP=4: 模型权重分片到4个GPU
...
echo "========================================"
echo "DFlash TP=4 + TurboQuant 测试 - 5轮对话"
echo "========================================"
...
# 创建测试脚本
cat > /tmp/test_dflash_tp4_5turns.py << 'PYEOF'
#!/usr/bin/env python3
"""DFlash TP=4 测试 - 5轮对话"""
...
def main():
    ...
    # - 注意力: 4x 分片
    # - FFN/MoE: 4x 分片
    # - AWQ 量化模型: 每个 GPU 约 4-5GB (总模型 ~20GB AWQ)
    #
    # TurboQuant KV cache:
    # - K: 4-bit QJL4 量化
    # - V: 2-bit MSE 量化
    # - KV cache 显存减少约 75%
    #
    # cache_max_entry_count=0.2: 保守的KV cache分配
    # - 约 1GB GPU 显存给 KV cache
    engine_config = PytorchEngineConfig(
        tp=4,  # TP=4 张量并行
        ep=1,  # 不使用 EP
        session_len=1024,
        cache_max_entry_count=0.2,  # 保守的KV cache
        max_batch_size=1,
        block_size=16,
        eager_mode=True,
...
python3 /tmp/test_dflash_tp4_5turns.py "$(pwd)" "$MODEL_PATH" "${DRAFT_MODEL:-}"
