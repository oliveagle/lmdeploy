#!/bin/bash
# EP=4 + TP=4 + TurboQuant 测试 (无 DFlash)
#
# 不使用 DFlash draft model，只测试主模型能否运行

set -e

echo "========================================"
echo "EP=4 + TP=4 测试 (无 DFlash)"
echo "========================================"
...
# 创建测试脚本
cat > /tmp/test_ep4_tp4_simple.py << 'PYEOF'
#!/usr/bin/env python3
"""EP=4 + TP=4 简单测试"""
...
python3 /tmp/test_ep4_tp4_simple.py "$(pwd)" "$MODEL_PATH"
