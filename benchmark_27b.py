#!/usr/bin/env python3
"""Benchmark Qwen3.6-27B-AWQ on LMDeploy main branch using TurboMind."""
...
def main():
    ...
    try:
        pipe = pipeline(
            ...
            backend_config=TurbomindEngineConfig(
                tp=1,
                cache_max_entry_count=0.5,  # Lower to save memory for 27B
                model_format='awq',
            )
        )
        print("OK")
    ...
    # Save results
    output_file = f'benchmark_27b_turbomind_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    with open(output_file, 'w') as f:
        json.dump({'date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'gpu': gpu_info,
                    'model': model_name, 'backend': 'turbomind', 'results': results}, f, indent=2)
