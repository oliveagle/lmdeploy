#!/bin/bash
# DFlash EP=4 + TP=4 + TurboQuant 测试 - 5轮对话 (带 empty_init)
#
# 使用 empty_init 延迟加载权重，减少初始化时的显存峰值

set -e

echo "========================================"
echo "DFlash EP=4 + TP=4 测试 (empty_init)"
echo "========================================"
...
echo "  EP: 4, TP: 4 (moe_tp)"
echo "  GPUs: 0,1,2,3"
echo "  empty_init: true (延迟加载权重)"
echo ""

# 创建测试脚本
cat > /tmp/test_dflash_empty_init.py << 'PYEOF'
#!/usr/bin/env python3
"""DFlash EP=4 + TP=4 测试 - empty_init"""
...
os.environ['LMDEPLOY_EXECUTOR_BACKEND'] = 'ray'  # empty_init 需要 ray
...
def main():
    print("=" * 70)
    print("DFlash EP=4 + TP=4 测试 (empty_init)")
    print("=" * 70)
    ...
    print(f"  EP: 4, TP: 4 (moe_tp)")
    print(f"  KV Cache 量化: TurboQuant (42)")
    print(f"  empty_init: true")
    ...
    # 关键配置: 使用 empty_init 延迟加载权重
    engine_config = PytorchEngineConfig(
        tp=1,  # 基础 tp=1
        ep=4,  # EP=4 专家并行
        attn_tp_size=1,  # 注意力不分片
        moe_tp_size=4,  # MoE FFN 维度分片到4个GPU
        session_len=1024,
        cache_max_entry_count=0.3,  # 保守的KV cache
        max_batch_size=1,
        block_size=16,
        eager_mode=True,
        quant_policy=QuantPolicy.TURBO_QUANT,  # TurboQuant KV cache量化
        empty_init=True,  # 关键：延迟加载权重，减少初始化峰值显存
        distributed_executor_backend='ray',  # empty_init 需要 ray 后端
    )
...
python3 /tmp/test_dflash_empty_init.py "$(pwd)" "$MODEL_PATH" "${DRAFT_MODEL:-}"
