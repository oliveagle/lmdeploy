...
def setup_paths():
    ...
    # Add lmdeploy lib directory to sys.path
    # This is needed for the Rust bridge to find the lmdeploy modules
    import os.path as osp

    # Get the directory containing this script
    script_dir = osp.dirname(osp.abspath(__file__))
    lmdeploy_dir = osp.dirname(script_dir)  # Parent directory of turbomind
    lmdeploy_root = osp.dirname(lmdeploy_dir)  # Parent of lmdeploy

    # Add paths to find lmdeploy modules
    for path in [lmdeploy_root, lmdeploy_dir, script_dir]:
        if path not in sys.path:
            sys.path.insert(0, path)

    # Add lmdeploy lib directory if it exists
    for possible_lib in [
        osp.join(lmdeploy_dir, 'lib'),
        osp.join(lmdeploy_root, 'lib'),
    ]:
        if osp.exists(possible_lib) and possible_lib not in sys.path:
            sys.path.insert(0, possible_lib)
...
# Wrap stdout to only pass JSON through
_original_stdout = sys.stdout
class JsonOnlyStdout:
    """Only allow JSON responses to pass to the real stdout, everything else goes to stderr."""
    def write(self, text):
        stripped = text.strip()
