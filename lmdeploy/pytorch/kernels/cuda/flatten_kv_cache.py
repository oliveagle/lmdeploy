...
def _flatten_kv_cache(
    ...
    vc_ptrs = (
        vc_ptr
        + b_off * stride_vcb
        + offs_bs[:, None] * stride_vcs
        + head_id * stride_vch
        + offs_dv[None, :] * stride_vcd
    )
    ko_ptrs = (
        ko_ptr + head_id * stride_koh + (start_loc + offs_obs[:, None]) * stride_kos + offs_dk[None, :] * stride_kod
    )
    vo_ptrs = (
        vo_ptr + head_id * stride_voh + (start_loc + offs_obs[:, None]) * stride_vos + offs_dv[None, :] * stride_vod
    )
...
def _flatten_kv_cache_quant(
    ...
    ko_ptrs = (
        ko_ptr + head_id * stride_koh + (start_loc + offs_obs[:, None]) * stride_kos + offs_dok[None, :] * stride_kod
    )
    vo_ptrs = (
        vo_ptr + head_id * stride_voh + (start_loc + offs_obs[:, None]) * stride_vos + offs_dov[None, :] * stride_vod
    )
...
def flatten_kv_cache(
    ...
    Uses a global buffer cache to reuse intermediate tensors across layers,
    reducing peak memory pressure from O(num_layers * buffer_size) to O(buffer_size).
    """
    from lmdeploy.pytorch.backends.cuda.attention.buffer_cache import get_or_allocate
    ...
    if flatten_kv_layout == "hsd":
        k_shape = (num_heads, out_size, k_head_dim)
        k_states = get_or_allocate(device, "flatten_k", k_shape, out_dtype, "hsd")
        if quant_policy == QuantPolicy.NONE and shared_kv:
            v_states = k_states[..., :v_head_dim]
            v_head_dim = 0
        else:
            v_shape = (num_heads, out_size, v_head_dim)
            v_states = get_or_allocate(device, "flatten_v", v_shape, out_dtype, "hsd")
        ...
        k_shape = (out_size, num_heads, k_head_dim)
        k_states = get_or_allocate(device, "flatten_k", k_shape, out_dtype, "shd")
        if quant_policy == QuantPolicy.NONE and shared_kv:
            v_states = k_states[..., :v_head_dim]
            v_head_dim = 0
        else:
            v_shape = (out_size, num_heads, v_head_dim)
            v_states = get_or_allocate(device, "flatten_v", v_shape, out_dtype, "shd")
