#!/usr/bin/env python3
"""
LMDeploy PyTorch Backend + AWQ MoE 单卡测试
验证 CUDA 避法内存访问问题是否修复
"""
import os
import sys
import time
...
def test_pytorch_awq_single_gpu():
    """测试 PyTorch Backend + AWQ MoE (单卡)"""
    print("=" * 70)
    print("LMDeploy PyTorch Backend + AWQ MoE 单卡测试")
    print("=" * 70)
...
def main():
    print("=" * 70)
    print("AWQ MoE 修复验证测试 (单卡)")
    print("=" * 70)
    print("目的: 验证 topk_ids 验证修复是否解决了 CUDA 非法内存访问问题")
    print("注意: 单卡测试避免 NCCL P2P 问题")
    print()

    success = test_pytorch_awq_single_gpu()
