#!/bin/bash
# DFlash EP=4 + TP=4 + TurboQuant 测试 - 5轮对话
#
# 使用 EP+TP 组合减少显存占用:
# - EP=4: 256个专家分到4个GPU，每卡64个专家
# - TP=4 (moe_tp): FFN维度也分片到4个GPU
# - EP*TP = 4*4 = 16x 显存缩减（MoE权重部分）
# - TurboQuant KV cache: K=4bit QJL4, V=2bit MSE

set -e

echo "========================================"
echo "DFlash EP=4 + TP=4 测试 - 5轮对话"
echo "========================================"
...
def main():
    ...
    # - moe_tp=4: FFN维度也分片到4个GPU (4x 显存缩减)
    # - EP * moe_tp = 16x 总缩减
    # - MoE权重从 ~34GB/卡 → ~12GB/卡 (无TP) → ~3GB/卡 (TP=4)
    #
