//! Rust Server Prefill/Decode Benchmark - Unified (4k, 8k, 16k)
//!
//! 与 Python TurboMind benchmark 完全一致：
//! - 输入长度: 4096, 8192, 16384 (4k, 8k, 16k)
//! - 输出长度: 512 tokens
//! - 预热: 2 次，测量: 5 次
//! - 并发度: 1 (串行测试)
//!
//! Usage:
//!     cargo run --bin prefill_benchmark_4_8_16k [--model /path/to/model] [--output results.json]
...
#[derive(Parser, Debug)]
#[command(name = "prefill_benchmark_4_8_16k")]
#[command(about = "Unified prefill/decode benchmark (4k, 8k, 16k) matching Python methodology")]
struct Args {
    /// Model path
    #[arg(long)]
    model: Option<String>,
...
struct ContextSummary {
    ...
    ttft_ms_median: f64,
    prefill_tps_avg: f64,
    prefill_tps_median: f64,
    decode_tps: f64,
    total_time_ms_median: f64,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n{}", "=".repeat(80));
    println!("LMDeploy Rust Server Benchmark (统一测试方法 - 4K, 8K, 16K)");
    println!("{}", "=".repeat(80));
    println!("Model:   {}", model_path);
    println!("Backend: turbomind (C++)");
    println!("Input:   4096, 8192, 16384 (4K, 8K, 16K)");
    println!("Output:  512 tokens");
    ...
    for &(label, target_tokens) in TEST_CONTEXTS {
        ...
        if !context_results.is_empty() {
            ...
            println!(
                "| Avg TTFT: {:.2}ms, Prefill: {:.1} tok/s, Decode: {:.1} tok/s",
                avg_ttft, avg_prefill, avg_decode
            );
        }
    }
    ...
    for &input_len in &[4096, 8192, 16384] {
        ...
        if !context_results.is_empty() {
            let avg_ttft = context_results.iter().map(|r| r.ttft_ms).sum::<f64>()
                / context_results.len() as f64;
            let median_ttft = {
                let mut sorted: Vec<f64> = context_results.iter().map(|r| r.ttft_ms).collect();
            ...
            let avg_prefill = context_results.iter().map(|r| r.prefill_tps).sum::<f64>()
                / context_results.len() as f64;
            let median_prefill = {
                let mut sorted: Vec<f64> = context_results.iter().map(|r| r.prefill_tps).collect();
                sorted.sort_by(|a, b| a.partial_cmp(b).unwrap());
                sorted[sorted.len() / 2]
            };
            let avg_decode = context_results.iter().map(|r| r.decode_tps).sum::<f64>()
                / context_results.len() as f64;
            let median_total = {
                let mut sorted: Vec<f64> = context_results.iter().map(|r| r.total_time_ms).collect();
                sorted.sort_by(|a, b| a.partial_cmp(b).unwrap());
                sorted[sorted.len() / 2]
            };
            ...
            summaries.push(ContextSummary {
                ...
                ttft_ms_median: median_ttft,
                prefill_tps_avg: avg_prefill,
                prefill_tps_median: median_prefill,
                decode_tps: avg_decode,
                total_time_ms_median: median_total,
            });
        }
    }
    ...
    let output_path = args.output.unwrap_or_else(|| {
        format!("results/prefill_benchmark_rust_4_8_16k_{}.json", chrono::Utc::now().format("%Y%m%d_%H%M%S"))
    });
