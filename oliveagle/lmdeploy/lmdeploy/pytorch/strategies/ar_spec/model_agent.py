...
class ARSpecModelAgentStrategy(ModelAgentStrategy):
    ...
    def slice_extra_inputs(self, extra_inputs: ARSpecExtraInputs, model_inputs: ModelInputs,
        ...
        logger.info(f'slice_extra_inputs: target_hidden_states={type(extra_inputs.target_hidden_states)}, '
                   f'model_outputs keys={list(model_outputs.keys())}, is_decoding={model_inputs.is_decoding}')
    ...
    def update_prefill_for_next_step(
        ...
        # For DFlash - check if we need prefill mode instead of decoding
        has_target_hidden = hasattr(model_inputs, 'target_hidden_states') and model_inputs.target_hidden_states is not None
        use_prefill_mode = has_target_hidden  # DFlash uses prefill mode

        inputs = get_model_inputs_next_decoding(model_inputs,
                                                next_token_ids,
                                                max_q_seqlen=max_q_seqlen,
                                                model_metas=model_metas,
                                                is_decoding=not use_prefill_mode)

        # Preserve target_hidden_states and target_position_ids for DFlash
        if has_target_hidden:
            inputs.target_hidden_states = model_inputs.target_hidden_states
            inputs.target_position_ids = getattr(model_inputs, 'target_position_ids', None)
