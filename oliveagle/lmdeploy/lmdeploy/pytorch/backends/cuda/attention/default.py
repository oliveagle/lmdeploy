...
class TritonAttentionImpl(AttentionImpl[TritonAttentionMetadata]):
    ...
    def _get_max_q_seqlen(
        ...
        """Get max q seqlen."""
        # First check metadata's max_q_seqlen (set by update_step_context)
        # This is important for speculative decoding where is_decoding may be True
        # but we're processing multiple tokens
        if attn_metadata.max_q_seqlen is not None:
            return attn_metadata.max_q_seqlen
        if attn_metadata.is_decoding:
            return self.block_sparse_size
        else:
            return query.numel() // (query.size(-1) * query.size(-2))
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Note: For speculative decoding, is_decoding may be True but we need
        # to process multiple tokens (draft tokens + bonus). In this case,
        # we must use prefill mode since paged attention only supports single token.
        # CRITICAL: max_q_seqlen must be correctly set in attn_metadata!
        is_decoding = attn_metadata.is_decoding and max_q_seqlen == 1
        if attn_metadata.is_decoding:
            logger.info(f'Dispatch: is_decoding={attn_metadata.is_decoding}, '
                       f'max_q_seqlen={max_q_seqlen}, '
                       f'routing_to={"decoding" if is_decoding else "prefill"}')
        if is_decoding:
