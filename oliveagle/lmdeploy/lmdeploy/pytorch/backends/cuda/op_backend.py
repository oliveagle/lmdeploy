...
class CudaOpsBackend(DefaultOpsBackend):
    ...
    def update_step_context(cls, step_context):
        ...
        # Calculate max_q_seqlen
        max_q_seqlen = q_seqlens.max().item() if q_seqlens.numel() > 0 else 1

        # For multi-token decoding (speculative decoding verification),
        # we need to treat it like prefill even though is_decoding=True
        is_multi_token = max_q_seqlen > 1
        treat_as_prefill = is_multi_token
        ...
        # For multi-token decoding, we need kv_start_loc and kv_flatten_size like prefill
        if treat_as_prefill:
        ...
        if step_context.is_decoding and not is_multi_token:
            if use_flash_mla:
                ...
                # DFlash uses custom attention, not FlashAttention-3
                # Check if the model is DFlash (has aux_hidden_state_layers)
                hf_config = step_context.model_config.hf_config
                is_dflash = (hasattr(hf_config, 'aux_hidden_state_layers') and
                             len(hf_config.aux_hidden_state_layers) > 1)
                if not is_dflash:
    ...
    def build_graph_runner(model: torch.nn.Module, model_config: ModelConfig, cache_config: CacheConfig,
        ...
        if len(warmup_mgr._warmup_calls) > 0:
            logger.info('Skipping warmup to avoid Triton kernel compilation issues on V100')
        else:
            warmup_mgr.warmup(warmup_meta)
