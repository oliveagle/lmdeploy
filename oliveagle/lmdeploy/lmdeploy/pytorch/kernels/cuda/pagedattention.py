...
# SM_70 (V100) compatibility - Triton 3.2.0 has LLVM compilation errors on V100
_IS_SM70 = torch.cuda.is_available() and torch.cuda.get_device_properties(0).major == 7 and torch.cuda.get_device_properties(0).minor == 0

if _IS_SM70:
    logger.warning('SM_70 (V100) detected: paged attention will use PyTorch fallback to avoid Triton compilation errors')

    def _flash_attn_with_kvcache_sm70(
        q: Tensor,
        k_cache: Tensor,
        v_cache: Tensor,
        ...
        """PyTorch fallback for paged attention on V100."""
        batch_size = q.size(0)
        num_heads = q.size(1)
        head_dim = q.size(2)
        kv_head_size = v_cache.size(-1)

        # Debug for DFlash
        import os
        _DEBUG = os.environ.get('DFLASH_DEBUG', '0') == '1'
        if _DEBUG:
            print(f'[DFlash DBG] paged_attn: q={q.shape}, k_cache={k_cache.shape}, page_table={page_table.shape}')
            print(f'[DFlash DBG]   cache_seqlens={cache_seqlens}, block_size={k_cache.size(1)}')
            for b in range(batch_size):
                print(f'[DFlash DBG]   batch {b}: seq_len={cache_seqlens[b].item()}, page_table={page_table[b]}')

        o = q.new_empty(batch_size, num_heads, head_dim)

        for b in range(batch_size):
            seq_len = cache_seqlens[b].item()
            pages = page_table[b, :((seq_len + page_table.size(1) - 1) // page_table.size(1))]

            # Gather KV from paged cache
            k_list = []
            v_list = []
            for i in range(pages.size(0)):
                if i * k_cache.size(1) >= seq_len:
                    break
                page_idx = pages[i].item()
                if page_idx < 0 or page_idx >= k_cache.size(0):
                    break
                tokens_in_page = min(k_cache.size(1), seq_len - i * k_cache.size(1))
                k_list.append(k_cache[page_idx, :tokens_in_page])
                v_list.append(v_cache[page_idx, :tokens_in_page])

            k_ctx = torch.cat(k_list, dim=0)  # [seq_len, num_kv_heads, head_dim]
            v_ctx = torch.cat(v_list, dim=0)

            # Expand for GQA
            num_kv_heads = k_ctx.size(1)
            num_kv_heads = k_ctx.size(1)
            num_q_heads = q.size(1)
            num_kv_groups = num_q_heads // num_kv_heads

            k_ctx = k_ctx.permute(1, 0, 2).contiguous()  # [num_kv_heads, seq_len, head_dim]
            v_ctx = v_ctx.permute(1, 0, 2).contiguous()  # [num_kv_heads, seq_len, head_dim]

            # Expand KV for GQA
            k_ctx = k_ctx.repeat_interleave(num_kv_groups, dim=0)  # [num_q_heads, seq_len, head_dim]
            v_ctx = v_ctx.repeat_interleave(num_kv_groups, dim=0)

            # Q: [seq_len_q, num_q_heads, head_dim]
            q_b = q[b].permute(1, 0, 2).contiguous()

            # Compute attention
            if softcap > 0:
                attn_logits = torch.matmul(q_b, k_ctx.transpose(1, 2)) * softmax_scale
                attn_logits = torch.tanh(attn_logits / softcap) * softcap
            else:
                attn_logits = torch.matmul(q_b, k_ctx.transpose(1, 2)) * softmax_scale

            if alibi_slopes is not None:
                # Simple ALiBi approximation for V100
                pass

            attn_weights = torch.softmax(attn_logits, dim=-1)
            out = torch.matmul(attn_weights, v_ctx)  # [num_q_heads, seq_len_q, head_dim]
            o[b] = out.permute(1, 0, 2)  # [seq_len_q, num_q_heads, head_dim]
    ...
    def flash_attn_with_kvcache(
        ...
        """Paged Attention forward - with V100 fallback."""
        if kv_layout == 'bshd':
        ...
        return _flash_attn_with_kvcache_sm70(
            q, k_cache, v_cache, cache_seqlens, page_table,
            softmax_scale, softcap, window_size,
            alibi_slopes, k_scales_zeros, v_scales_zeros, quant_policy,
        )
