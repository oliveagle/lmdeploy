...
def draft_model_forward(
    ...
    with torch.cuda.stream(stream), step_ctx_manager(model.ctx_mgr):
        ...
        cache_config = None if cache_engine is None else cache_engine.cache_config
        ...
        with ctx_mgr.context(context):
            ...
            # DFlash: pass target_hidden_states to model forward
            if hasattr(inputs, 'target_hidden_states') and inputs.target_hidden_states is not None:
                input_dict['target_hidden_states'] = inputs.target_hidden_states
