...
class SpecModelAgent(BaseSpecModelAgent):
    ...
    def build_graph_runner(self):
        """Build graph runner."""
        # DFlash is non-autoregressive and doesn't use CUDA graphs
    ...
    def _rejection_sampling(self, next_token_ids, model_inputs: 'ModelInputs', extra_inputs):
        ...
        # DFlash: extra_inputs may be ARExtraInputs (no target_hidden_states)
        # DFlash doesn't need rejection sampling for the first pass
        if self.method == 'dflash':
            # Wrap extra_inputs to ARSpecExtraInputs with required fields
            if not isinstance(extra_inputs, ARSpecExtraInputs):
                extra_inputs = ARSpecExtraInputs(
                    next_token_ids=next_token_ids,
                    last_token_indices=last_token_indices,
                    num_rejected_tokens=num_rejected_tokens,
                    output_token_ids=output_token_ids,
                    target_hidden_states=getattr(extra_inputs, 'target_hidden_states', None),
                    target_position_ids=getattr(extra_inputs, 'target_position_ids', None),
                )
            # DFlash: just pass through, rejection sampling happens after
            ...
            # Ensure model_inputs has target_hidden_states for draft model
            model_inputs.target_hidden_states = new_extra_inputs.target_hidden_states
            model_inputs.target_position_ids = new_extra_inputs.target_position_ids
            # Debug: check if target_hidden_states is being set correctly
            from lmdeploy.utils import get_logger
            logger = get_logger('lmdeploy')
            logger.info(f'DFlash _rejection_sampling: target_hidden_states={type(new_extra_inputs.target_hidden_states)}, '
                       f'model_inputs.target_hidden_states={type(model_inputs.target_hidden_states)}')
            return model_inputs, new_extra_inputs
    ...
    async def _async_model_forward(self, inputs: ModelInputs, extra_inputs: ARSpecExtraInputs,
        ...
        # DFlash generates all tokens in one pass (no autoregressive loop)
        if self.method == 'dflash':
            draft_token_ids, model_metas, target_hidden_states = self.proposer.get_outputs(
                outputs, inputs, extra_inputs)
            # DFlash returns [num_spec_tokens, 1] as [batch, num_spec_tokens, 1] or [batch*num_spec_tokens, 1]
            # Reshape to [batch, num_spec_tokens]
            if draft_token_ids.dim() == 3:
                # [batch, num_spec_tokens, 1]
                output_draft_ids = draft_token_ids.squeeze(-1)
            elif draft_token_ids.size(0) > 1 and draft_token_ids.size(0) == self.num_spec_tokens:
                # [num_spec_tokens, 1]
                output_draft_ids = draft_token_ids.t().contiguous()
            else:
                # Fallback
                output_draft_ids = draft_token_ids.expand(-1, self.num_spec_tokens)
            return output_draft_ids

        # Standard autoregressive loop for EAGLE/DeepseekMTP
    ...
    def warmup(self, max_batches: int, target_model_config: ModelConfig):
        ...
        # DFlash uses prefill mode for parallel multi-token generation
        # So we warmup with is_decoding=False instead of True
        if self.method == 'dflash':
            # Warmup prefill with draft tokens (is_decoding=False)
            inputs = self.inputs_strategy.make_dummy(
                max_batches,
                is_decoding=False,
                device='cuda',
                vocab_size=self.model_config.vocab_size,
                max_q_seqlen=self.num_spec_tokens,
                target_hidden_size=target_hidden_size,
                target_dtype=self.model_config.dtype,
            )
            self._forward_impl(inputs)
            # Warmup single token prefill (for standard decoding)
            inputs = self.inputs_strategy.make_dummy(
                max_batches,
                is_decoding=False,
                device='cuda',
                vocab_size=self.model_config.vocab_size,
                max_q_seqlen=1,
                target_hidden_size=target_hidden_size,  # DFlash always uses concatenated hidden states
                target_dtype=self.model_config.dtype,
            )
            self._forward_impl(inputs)
            return
