...
class BaseSpecModelAgent:
    ...
    def update_main_model_outputs(self, output: Dict[str, torch.Tensor], model_inputs: ModelInputs):
        ...
        if 'aux_hidden_states' in output:
            ...
            logger.info(f'update_main_model_outputs: Found aux_hidden_states with shape={output["aux_hidden_states"].shape}')
            output['hidden_states'] = output.pop('aux_hidden_states')
        else:
            from lmdeploy.utils import get_logger
            logger = get_logger('lmdeploy')
            logger.warning(f'update_main_model_outputs: No aux_hidden_states in output! Keys={list(output.keys())}')
        return hidden_states, output
