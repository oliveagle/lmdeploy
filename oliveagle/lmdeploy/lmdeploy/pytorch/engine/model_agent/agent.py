...
class BaseModelAgent:
    ...
    def __init__(
        ...
        # spec decoding
        # For ar_spec methods like DFlash, use spec_model_config's strategy factory
        if specdecode_config is not None and specdecode_config.model_config is not None:
            spec_strategy_factory = build_strategy_factory(specdecode_config.model_config,
                                                          misc_config,
                                                          specdecode_config=specdecode_config)
            spec_inputs_strategy = spec_strategy_factory.build_model_inputs_strategy()
            spec_agent_strategy = spec_strategy_factory.build_model_agent_strategy()
        else:
            spec_inputs_strategy = self.inputs_strategy
            spec_agent_strategy = self.agent_strategy

        self.spec_agent = build_spec_agent(specdecode_config,
                                           backend_config,
                                           dist_ctx,
                                           spec_inputs_strategy,
                                           spec_agent_strategy,
                                           device=device)
        # sleep wakeup state
    ...
    def _postprocess_forward_output(self, output: dict, inputs: ModelInputs):
        ...
        # Process aux_hidden_states if present (for DFlash)
        if 'aux_hidden_states' in output:
            aux_hidden = output['aux_hidden_states']
            aux_hidden = self._slice_outs(aux_hidden[0], seq_length)[None]
            output['aux_hidden_states'] = aux_hidden
    ...
    async def _async_step(
        ...
        # Debug: check if extra_inputs has target_hidden_states
        if hasattr(extra_inputs, 'target_hidden_states'):
            logger.info(f'After slice_extra_inputs: target_hidden_states={type(extra_inputs.target_hidden_states)}')
        else:
            logger.warning(f'After slice_extra_inputs: extra_inputs has no target_hidden_states attribute, type={type(extra_inputs)}')
