#!/bin/bash
# 最终解决方案：使用 TP4 + INT4 KV Cache 而不是 TurboQuant
# TurboQuant 在 V100 上有问题

set -e

echo "========================================"
echo "TP=4 + INT4 KV Cache 测试"
echo "========================================"
...
echo "模型: $MODEL_PATH"
echo "配置: EP=1, TP=4, INT4 KV Cache"
echo "GPUs: 0,1,2,3"
echo ""

# 创建测试脚本
cat > /tmp/test_tp4_int4kv.py << 'PYEOF'
#!/usr/bin/env python3
"""TP=4 + INT4 KV Cache 测试"""
...
def main():
    ...
    engine_config = PytorchEngineConfig(
        ...
        block_size=64,
        eager_mode=True,
        quant_policy=QuantPolicy.INT4,  # INT4 KV Cache (更简单，V100 支持更好)
    )
...
python3 /tmp/test_tp4_int4kv.py "$(pwd)" "$MODEL_PATH"
